...
/**
 * Every console mutation goes through here: one form, one Server Action, one
 * place that renders the human sentence the server handed back.
 */

export type Action = (prev: ActionState, formData: FormData) => Promise<ActionState>;

const styles = {
  primary: "bg-clay text-white hover:bg-[#4a2576] border border-clay",
...
export function ActionForm({
  ...
  confirm?: string;
  className?: string;
  /** Drops the gap above the button, for one-button forms sitting inline. */
  compact?: boolean;
  footer?: ReactNode;
}) {
