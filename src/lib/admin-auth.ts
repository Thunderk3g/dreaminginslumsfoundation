...
/**
 * CMS auth, deliberately minimal: one shared password, no user table, no
 * Supabase Auth. A foundation with a handful of people does not need account
 * management, and every extra moving part here is another place to get
 * authorization wrong.
 *
 ...
 *
 * Node runtime only. No `server-only` import so the crypto here stays
 * unit-testable; `next/headers` already makes the module unusable from a Client
 * Component, and ADMIN_PASSWORD is not NEXT_PUBLIC_, so it can never reach the
 * browser bundle.
 */

export const ADMIN_COOKIE = "dis_admin";
...
export async function startSession(): Promise<void> {
  ...
  store.set(ADMIN_COOKIE, `${expiresAt}.${sign(expiresAt)}`, {
    ...
    // never stored by the browser over http://, which would make local sign-in
